...
export default function WeatherPanel({ lat, lng, onClose }) {
  ...
  useEffect(() => {
    ...
    url.searchParams.set("forecast_days", "2"); // today + tomorrow = 48 hours
  ...
  return (
    <div
      style={{
        ...
        width: 340, background: "rgba(0,0,0,0.88)",
        backdropFilter: "blur(10px)", color: "white",
        borderRadius: 12, zIndex: 2, fontFamily: "Arial",
    ...
    >
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        <div>
          <span style={{ fontSize: 13, fontWeight: "bold", color: "#34d399" }}>☁ Weather Data</span>
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 7, marginBottom: 16 }}>
                {[
                  ...
                  { label: "Wind", value: `${c.wind_speed_10m} m/s\n${windDir(c.wind_direction_10m)}`, icon: "💨" },
                  { label: "Precip.", value: `${c.precipitation} mm`, icon: "🌧️" },
                ].map(({ label, value, icon }) => (
                  <div key={label} style={{ background: "rgba(255,255,255,0.05)", borderRadius: 8, padding: "8px 6px", textAlign: "center" }}>
                    <div style={{ fontSize: 16 }}>{icon}</div>
                    <div style={{ fontSize: 10, color: "#64748b", marginTop: 2 }}>{label}</div>
                    <div style={{ fontSize: 11, fontWeight: "bold", marginTop: 2, whiteSpace: "pre-line" }}>{value}</div>
                  </div>
                ))}
              </div>

              {/* Hourly breakdown header + nav */}
              <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: 12, marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <span style={{ fontSize: 11, color: "#475569", letterSpacing: "0.06em" }}>HOURLY BREAKDOWN</span>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        border: isNow ? "1px solid rgba(52,211,153,0.2)" : "1px solid transparent",
                      }}
                    >
                      {/* Time */}
                      <span style={{ fontSize: 11, color: isNow ? "#34d399" : "#64748b", fontWeight: isNow ? "bold" : "normal" }}>
                        {isNow ? "Now" : `${String(hour).padStart(2, "0")}:00`}
                      </span>

                      {/* Icon */}
                      <span style={{ fontSize: 15, textAlign: "center" }}>{WMO_ICON[hCode] ?? "🌡️"}</span>
                      ...
                      {/* Precip mm */}
                      <span style={{ fontSize: 10, color: precip > 0 ? "#7dd3fc" : "#334155", textAlign: "right" }}>
                        {precip > 0 ? `${precip}mm` : "—"}
                      </span>
                    </div>
                  );
              ...
              <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: 10, marginTop: 4 }}>
                <div style={{ fontSize: 11, color: "#475569", letterSpacing: "0.06em", marginBottom: 8 }}>WIND & HUMIDITY (HOURLY)</div>
                {hourSlice.map((timeStr, i) => {
                  const idx = windowStart + i;
                  const hour = new Date(timeStr).getHours();
                  const isNow = idx === currentIdx;
